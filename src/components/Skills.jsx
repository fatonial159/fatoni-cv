...
const Skills = ({ skills }) => {
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {skills.map((skill) => (
            <div key={skill.id} className="border border-gray-100 rounded-xl p-5 hover:border-blue-200 hover:shadow-md transition-all duration-300 bg-gray-50/50">
                <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                    <img src={skill.icon_url} alt={skill.name} className="w-10 h-10" onError={(e) => e.target.style.display = 'none'} />
                    <div>
                ...
                {skill.is_main && (
                    <span className="px-3 py-1 bg-blue-600 text-white text-xs font-bold rounded-full shadow-blue-200 shadow-lg">
                    Main
                    </span>
                )}
                </div>
